...
class DCN(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, cate_feature_size, field_size, numeric_feature_size,
                 embedding_size=8, deep_layers=[32, 32], dropout_deep=[0.5, 0.5, 0.5],
                 deep_layers_activation=tf.nn.relu, epoch=10, batch_size=256,
                 learning_rate=0.001, optimizer_type="adam", batch_norm=0, batch_norm_decay=0.995,
                 verbose=False, random_seed=2019, loss_type="logloss", eval_metric=roc_auc_score,
        ...
        # 输入到DCN中的输入维度是初始数值型特征维度+离散型变量嵌入后的维度
        ...
        self.optimazer_type = optimizer_type
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.dropout_keep_deep = tf.placeholder(tf.float32, shape=[None], name="drop_out_keep_deep")
            ...
            self.x0 = tf.concat([self.numeric_value, tf.reshape(self.embeddings, shape=[-1, self.field_size * self.embedding_size])],
                                axis=1)
            ...
            for l in range(self.cross_layer_num):
                x_l = tf.tensordot(tf.matmul(self._x0, x_l, transpose_b=True),
                                   self.weights["cross_layer_%d" % l], 1) + \
    ...
    def _initialize_weights(self):
        ...
        # embedding层
        # 只有类别型变量需要embedding，连接权重是类别型变量个数*embedding向量的大小
        ...
        # embedding后和连续向量组合连接第一层神经网络
        weights['deep_layer_0'] = tf.Variable(
            np.random.normal(loc=0, scala=glorot, size=(self.total_size, self.deep_layers[0])), dtype=np.float32
        )
        # 每一个神经元共用一个bias
    ...
    def predict(self, X_i, X_v, X_v2, y):
        feed_dict = {
            self.feat_index: X_i,
            self.feat_value: X_v,
            self.numeric_value: X_v2,
            self.label: y,
    ...
    def fit_on_batch(self, X_i, X_v, X_v2, y):
        feed_dict = {
            self.feat_index: X_i,
            self.feat_value: X_v,
            self.numeric_value: X_v2,
    ...
    def fit(self, cate_Xi_train, cate_Xv_train, numeric_Xv_train, y_train,
            cate_Xi_valid=None, cate_Xv_valid=None, numeric_Xv_valid=None,
        ...
        for epoch in range(self.epoch):
            ...
            for i in range(total_batch):
                cate_Xi_batch, cate_Xv_batch, numeric_Xv_batch, y_batch = self.get_batch(
                    cate_Xi_train, cate_Xv_train, numeric_Xv_train, y_train, self.batch_size, i
                )
